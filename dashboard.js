...
const STANDUP_DEADLINE_HOUR = 10; // 10 AM
const IN_PROGRESS_WINDOW_MIN = 15; // If last submit was < 15 min ago, angel "currently rounding"
...
async function loadDashboard() {
  try {
    ...
    // Load full week of data (we need it for the Weekly tab anyway)
...
function renderBoard(today) {
  ...
  // Currently rounding — angels who submitted in the last 15 min but aren't done
  renderCurrentlyRounding(rounderAngels, todayRounds);

  // Missing angels banner (only after 10 AM)
...
function changeWeek(delta) {
  ...
  if (currentWeekOffset > 0) currentWeekOffset = 0; // no future weeks
  renderWeekly();
}

function getWeekDates(offset) {
  // Returns Monday-Friday dates for the given week offset (0 = this week)
  const today = new Date();
...
function renderWeekly() {
  ...
  // Disable "next" if we're on the current week
